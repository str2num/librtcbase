...
#include "byte_order.h"  // for RTC_ARCH_CPU_LITTLE_ENDIAN.
#include "md5.h"
...
static void byte_reverse(uint32_t* buf, int len) {
    for (int i = 0; i < len; ++i) {
        buf[i] = rtcbase::get_le32(&buf[i]);
    }
}
#endif
...
void MD5_update(MD5Context* ctx, const uint8_t* buf, size_t len) {
    // Update bitcount.
